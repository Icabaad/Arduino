...
//Sen.se 
const long int device_id = 16127; // This is your device's ID
const unsigned long int feed_43584 = 43584; // This is the feed "comms-room-temperature"
int val = 0;
char buffer[30];
...
//  }
  ...
  if (timer >= 30) {
            ...
            Serial.println("POST value for feed comms-room-temperature");
      
      
      val = barometerTemp;
      
      sprintf(buffer,"%d",val);
      Serial.println("Posting in feed comms-room-temperature");
      if(pf.postSense(feed_43584, buffer)) {
        Serial.println("POST in feed comms-room-temperature succeeded");
      } else {
        Serial.println("POST in feed comms-room-temperature failed");
      }
